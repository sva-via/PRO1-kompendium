...
```
Her har Song en relation til Time. Da Time bruges som varighed i sangen, ønsker der ikke at dele samme objekt på tværs, og derfor ligner relationen composition. Hvis Time er en immutable klasse, så kan relationen moddeleres som aggregation.
...
```
Her er spørgsmålet, om Person skal eje MyDate. Hvis MyDate er immutable, er deling ofte uproblematisk. Hvis MyDate er mutable, bør du overveje defensive copying (fra Kapitel 4) i både konstruktør og getter for at undgå representation exposure - og dette svarer præcist til composition.
